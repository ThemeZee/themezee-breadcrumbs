 ...
 *
 *
 * The following code is a derivative work of the code from the Breadcrumb Trail plugin,
...
class TZBC_Breadcrumb_Trail {
	 ...
	 *     @type array     $labels         Text labels. @see TZBC_Breadcrumb_Trail::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see TZBC_Breadcrumb_Trail::set_post_taxonomy()
	...
	public function trail() {
		...
		$breadcrumb = apply_filters( 'themezee_breadcrumbs', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$defaults = array(
			...
			'archives'            => esc_html__( 'Archives', 'themezee-breadcrumbs' ),
			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'              => esc_html__( 'Search results for &#8220;%s&#8221;', 'themezee-breadcrumbs' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s', 'themezee-breadcrumbs' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s', 'themezee-breadcrumbs' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s', 'themezee-breadcrumbs' ),
		...
		$this->labels = apply_filters( 'themezee_breadcrumbs_labels', wp_parse_args( $this->args['labels'], $defaults ) );
	}
	...
	protected function set_post_taxonomy() {
		...
		$this->post_taxonomy = apply_filters( 'themezee_breadcrumbs_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
	}
	...
	protected function add_items() {
		...
		$this->items = array_unique( apply_filters( 'themezee_breadcrumbs_items', $this->items, $this->args ) );
	}
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'themezee-breadcrumbs' ) ) );
		}
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'themezee-breadcrumbs' ) ) );
		}
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'themezee-breadcrumbs' ) ) );
		}
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'themezee-breadcrumbs' ) ) );
